...
//Twin Karmakharm BEGIN CHANGE
//added functions for setting the partitioning parameters of the messages
<xsl:for-each select="gpu:xmodel/xmml:messages/gpu:message">

	<xsl:if test="gpu:partitioningSpatial">

		<!--void fgClear_<xsl:value-of select="xmml:name"/>_partitioning_params()-->
		<!--{-->
			<!--cudaFree(h_<xsl:value-of select="xmml:name"/>_partition_matrix.start);-->
			<!--cudaFree(h_<xsl:value-of select="xmml:name"/>_partition_matrix.end);-->
			<!--cudaFree(d_<xsl:value-of select="xmml:name"/>_partition_matrix);-->
			<!--d_<xsl:value-of select="xmml:name"/>_partition_matrix = 0;-->
			...
			<!--h_<xsl:value-of select="xmml:name"/>s = (xmachine_message_<xsl:value-of select="xmml:name"/>_list*)malloc(message_<xsl:value-of select="xmml:name"/>_SoA_size);-->
			...
			<!--gpuErrchk(cudaMemcpy(d_<xsl:value-of select="xmml:name"/>s_swap, h_<xsl:value-of select="xmml:name"/>s , message_<xsl:value-of select="xmml:name"/>_SoA_size, cudaMemcpyHostToDevice));-->
		...
		{
			<!--if(d_<xsl:value-of select="xmml:name"/>_partition_matrix != 0)-->
			<!--fgClear_<xsl:value-of select="xmml:name"/>_partitioning_params();-->
		...
		//Set the grid size
			xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size = h_message_<xsl:value-of select="xmml:name"/>_partitionDim.x * h_message_<xsl:value-of select="xmml:name"/>_partitionDim.y * h_message_<xsl:value-of select="xmml:name"/>_partitionDim.z;


			/* <xsl:value-of select="xmml:name"/> Partitioning allocation (GPU) */
		h_<xsl:value-of select="xmml:name"/>_partition_matrix = (xmachine_message_<xsl:value-of select="xmml:name"/>_PBM*) malloc( sizeof(xmachine_message_<xsl:value-of select="xmml:name"/>_PBM));


		gpuErrchk( cudaMalloc( (void**) &amp;h_<xsl:value-of select="xmml:name"/>_partition_matrix->start, xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size*sizeof(int)));
		gpuErrchk( cudaMalloc( (void**) &amp;h_<xsl:value-of select="xmml:name"/>_partition_matrix->end_or_count, xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size*sizeof(int)));
		...
		<!--void fgLoadDefault_<xsl:value-of select="xmml:name"/>_partitioning_params()-->
		<!--{-->
			<!--if(d_<xsl:value-of select="xmml:name"/>_partition_matrix != 0)-->
			<!--fgClear_<xsl:value-of select="xmml:name"/>_partitioning_params();-->

			<!--fgSet_<xsl:value-of select="xmml:name"/>_partitioning_params((float)<xsl:value-of select="gpu:partitioningSpatial/gpu:radius"/>,-->
			<!--(float)<xsl:value-of select="gpu:partitioningSpatial/gpu:xmin"/>,-->
			<!--(float)<xsl:value-of select="gpu:partitioningSpatial/gpu:xmax"/>,-->
			<!--(float)<xsl:value-of select="gpu:partitioningSpatial/gpu:ymin"/>,-->
			<!--(float)<xsl:value-of select="gpu:partitioningSpatial/gpu:ymax"/>,-->
			<!--(float)<xsl:value-of select="gpu:partitioningSpatial/gpu:zmin"/>,-->
			<!--(float)<xsl:value-of select="gpu:partitioningSpatial/gpu:zmax"/> );-->
		<!--}-->
...
void <xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>(cudaStream_t &amp;stream){
	...
	gpuErrchk( cudaBindTexture(&amp;tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_start_byte_offset, tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_start, h_<xsl:value-of select="xmml:name"/>_partition_matrix->start, sizeof(int)*xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size));
	...
	gpuErrchk( cudaBindTexture(&amp;tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end_or_count_byte_offset, tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end_or_count, h_<xsl:value-of select="xmml:name"/>_partition_matrix->end_or_count, sizeof(int)*xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size));
  h_tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end_or_count_offset = (int)tex_xmachine_message_<xsl:value-of select="xmml:name"/>_pbm_end_or_count_byte_offset / sizeof(int);
	...
	gpuErrchk( cudaMemset( (void*) h_<xsl:value-of select="xmml:name"/>_partition_matrix->start, 0, sizeof(int) * xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size));
		gpuErrchk( cudaMemset( (void*) h_<xsl:value-of select="xmml:name"/>_partition_matrix->end_or_count, 0, sizeof(int) * xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size));
...
extern void reset_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>_count()
{
	...
	/****
	UPLOADING AND DOWNLOADING AGENT DATA
	***/
	<xsl:for-each select="gpu:xmodel/xmml:xagents/gpu:xagent/xmml:states/gpu:state">
		<xsl:variable name="agentName" select="../../xmml:name"/>
		<xsl:variable name ="agentState" select="xmml:name"/>
		<xsl:variable name="agentDataType">xmachine_memory_<xsl:value-of select="$agentName"/></xsl:variable>
		/************* UPLOAD DATA FOR AGENT <xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/> ***********/

		__global__ void uploadXAgentDataKernel_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>(<xsl:value-of select="$agentDataType"/>_list* agentList, <xsl:value-of select="$agentDataType"/>* agentData, int offset, int count )
		{
		//global thread index
		...
		{
		<xsl:for-each select="../../xmml:memory/gpu:variable">
			agentList-><xsl:value-of select="xmml:name"/>[offset + index]  = agentData[index].<xsl:value-of select="xmml:name"/>;</xsl:for-each>
		}
		}

		void fgUploadAgent_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>( <xsl:value-of select="$agentDataType"/>* agentData , int offset, int count )
		{
		//Check that mem copy is valid, return we try to write more than max number of agents
		if( count &lt; 1 || offset + count &gt; get_agent_<xsl:value-of select="$agentName"/>_MAX_count() ) return;

		//Memory allocation
		<xsl:value-of select="$agentDataType"/>* d_agentmem;
		size_t copySize =  sizeof(<xsl:value-of select="$agentDataType"/>) * count;
		LogCudaSafeCall(cudaMalloc( (void**) &amp;d_agentmem, copySize ));
		LogCudaSafeCall(cudaMemcpy( (void*)  d_agentmem , (void*) agentData, copySize, cudaMemcpyHostToDevice));

		//cuda block size
		int threads_per_tile = 128;
		int tile_size = (int) ceil((float)count/threads_per_tile);
		dim3 grid = dim3(tile_size, 1, 1);
		dim3 threads = dim3(threads_per_tile, 1, 1);

		//Upload to the agent
		uploadXAgentDataKernel_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>&lt;&lt;&lt; grid, threads >>>(get_device_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>_agents(), d_agentmem, offset, count );
		LogCudaCheckError("Kernel execution failed");

		LogCudaSafeCall(cudaDeviceSynchronize());

		//Memory de-allocation
		LogCudaSafeCall(cudaFree(d_agentmem));



		if( offset + count &gt; h_xmachine_memory_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>_count)
		h_xmachine_memory_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>_count = offset + count;
		...
		{
		fgUploadAgent_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>(agentData , h_xmachine_memory_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>_count, count );
		}


		/************* DOWNLOAD DATA FOR AGENT <xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/> ***********/

		__global__ void downloadXAgentDataKernel_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>(<xsl:value-of select="$agentDataType"/>_list* agentList, <xsl:value-of select="$agentDataType"/>* agentData, int offset, int count )
		{
		//global thread index
		int index = __mul24(blockIdx.x,blockDim.x) + threadIdx.x;
		...
		{
		<xsl:for-each select="../../xmml:memory/gpu:variable">
			agentData[index].<xsl:value-of select="xmml:name"/> = agentList-><xsl:value-of select="xmml:name"/>[index + offset];</xsl:for-each>
		}

		}

		void fgDownloadAgent_<xsl:value-of select="$agentName"/>_<xsl:value-of select="$agentState"/>( <xsl:value-of select="$agentDataType"/>* agentDataBuffer,  int offset, int count  )
		{
		//Check that mem copy is valid, return we try to write more than max number of agents
		if( offset + count &gt; get_agent_<xsl:value-of select="$agentName"/>_MAX_count() ) return;
		...
		//Download to temp var
		downloadXAgentDataKernel_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>&lt;&lt;&lt; grid, threads, 0 >>>(get_device_<xsl:value-of select="../../xmml:name"/>_<xsl:value-of select="xmml:name"/>_agents(), d_agentmem, offset, count);

		//Copy to host
		LogCudaSafeCall(cudaMemcpy( (void*) agentDataBuffer , (void*)  d_agentmem, copySize, cudaMemcpyDeviceToHost));
