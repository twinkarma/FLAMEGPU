...
<xsl:for-each select="gpu:xmodel/xmml:messages/gpu:message">
	...
	<xsl:if test="gpu:partitioningSpatial">
		...
		//Set the grid size
			...
			gpuErrchk(cudaMalloc(&amp;h_<xsl:value-of select="xmml:name"/>_partition_matrix.start, sizeof(int) * xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size));
			gpuErrchk(cudaMalloc(&amp;h_<xsl:value-of select="xmml:name"/>_partition_matrix.end_or_count, sizeof(int) * xmachine_message_<xsl:value-of select="xmml:name"/>_grid_size));
